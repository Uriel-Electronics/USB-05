...
class BluetoothManager: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    @Published var connectedDeviceName: String = "USB-05"
    @Published var modeNum: Int = 0
    
    @Published var OnTime: Int = 0
    @Published var OffTime: Int = 0
    @Published var OnTime2: Int = 0
    @Published var OffTime2: Int = 0
    
    @Published var Mode2OnTime: Int = 0
    @Published var Mode2OffTime: Int = 0
    @Published var Mode2OnTime2: Int = 0
    @Published var Mode2OffTime2: Int = 0
    
    @Published var Mode3OnTime: Int = 0
    @Published var Mode3OffTime2: Int = 0
    
    @Published var Mode4OnTime2: Int = 0
    @Published var Mode4OffTime: Int = 0
    
    @Published var Mode5OnTime: Int = 0
    @Published var Mode5OffTime: Int = 0
    
    @Published var Mode6OnTime: Int = 0
    @Published var Mode6OffTime: Int = 0
    @Published var Mode6OnTime2: Int = 0
    @Published var Mode6OffTime2: Int = 0
        ...
        if let data = characteristic.value {
            ...
            if response[0] == 0xDF {
                ...
                if calculatedChecksum == checksum {
                    ...
                    let modeNum = (mode & 0x07)// BIT0 to BIT2: 1 ~ 6 MODE
                    // let modeNum = mode
                    ...
                    DispatchQueue.main.async {
                        if modeNum == 1 {
                            if onTime >= 65476 {
                                self.OnTime = Int(onTime) - 65536
                            } else if (onTime >= 0 && onTime <= 60) {
                                self.OnTime = Int(onTime)
                            }
                            
                            if offTime >= 65476 {
                                self.OffTime = Int(offTime) - 65536
                            } else if (offTime >= 0 && offTime <= 60) {
                                self.OffTime = Int(offTime)
                            }
                            
                        } else if (modeNum == 2) {
                            if onTime >= 65476 {
                                self.Mode2OnTime = Int(onTime) - 65536
                            } else if (onTime >= 0 && onTime <= 60) {
                                self.Mode2OnTime = Int(onTime)
                            }
                            
                            if offTime >= 65476 {
                                self.Mode2OffTime = Int(offTime) - 65536
                            } else if (offTime >= 0 && offTime <= 60) {
                                self.Mode2OffTime = Int(offTime)
                            }
                            
                            self.Mode2OnTime2 = Int(onTime2)
                            self.Mode2OffTime2 = Int(offTime2)
                            
                        } else if (modeNum == 3) {
                            if onTime >= 65476 {
                                self.Mode3OnTime = Int(onTime) - 65536
                            } else if (onTime >= 0 && onTime <= 60) {
                                self.Mode3OnTime = Int(onTime)
                            }
                            self.Mode3OffTime2 = Int(offTime2)
                            
                        } else if (modeNum == 4) {
                            if offTime >= 65476 {
                                self.Mode4OffTime = Int(offTime) - 65536
                            } else if (offTime >= 0 && offTime <= 60) {
                                self.Mode4OffTime = Int(offTime)
                            }
                            
                            self.Mode4OnTime2 = Int(onTime2)
                        } else if (modeNum == 5) {
                            self.Mode5OnTime = Int(onTime)
                            self.Mode5OffTime = Int(offTime)
                        } else if (modeNum == 6) {
                            self.Mode6OnTime = Int(onTime)
                            self.Mode6OffTime = Int(offTime)
                            self.Mode6OnTime2 = Int(onTime2)
                            self.Mode6OffTime2 = Int(offTime2)
                        }
                    }

...
struct PeripheralSettingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if isLoading {
                VStack {
                    Text("연결 해제 중").font(Font.custom("Pretendard", size: 24).weight(.bold)).foregroundColor(.textLight).padding(.top, 40)
                        ...
                        VStack(spacing: 20) {
                            HStack(spacing: 50) {
                                VStack (alignment: .leading) {
                                    ...
                                    HStack(spacing: 10) {
                                        TextField(loadDeviceName(uuidString: bluetoothManager.connectedDeviceIdentifier) ?? "USB-05", text: $deviceName)
                                            .padding()
